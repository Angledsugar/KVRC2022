...
class MazeView2D:
    ...
    def __init__(self, maze_name="Maze2D", maze_file_path=None,
                 ...
                 has_loops=False, num_portals=0, num_waypoints = 5, enable_render=True):
        ...
        self.randomwaypoint = 0

        # Load a maze
        if maze_file_path is None:
            self.__maze = Maze(maze_size=maze_size, has_loops=has_loops, num_portals=num_portals, num_waypoints = 5)
        else:
        ...
        self.__entrance = np.array(self.maze_size) - np.array((6, 1)) # np.zeros(2, dtype=int)
        
        # Set the Goal
        self.__goal = np.array((5, 0)) # np.array(self.maze_size) - np.array((1, 1))
        self.__waypoint = {}
    ...
    def __draw_maze(self):
        ...
        # breaking the walls
        # print("maze | __draw_maze : ", self.maze.get_walls_status(self.maze.maze_cells[0, 0]))
    ...
    def __draw_waypoints(self, transparency=160):

        if self.__enable_render is False:
            return

        if self.randomwaypoint == 0:
        
            for x in range(len(self.maze.maze_cells)):
                for y in range (len(self.maze.maze_cells[x])):
                    if ((x == 5) and (y == 0)) or ((x == 5) and (y == 10)): break
                    
                    # check the which walls are open in each cell
                    wall_num = 0
                    walls_status = self.maze.get_walls_status(self.maze.maze_cells[x, y])
                    
                    if y > 0  : wall_num = self.maze.get_walls_status(self.maze.maze_cells[x, y-1])['S']
                    if x < 10 : wall_num = wall_num + self.maze.get_walls_status(self.maze.maze_cells[x+1, y])['W']
                    if y < 10 : wall_num = wall_num + self.maze.get_walls_status(self.maze.maze_cells[x, y+1])['N']
                    if x > 0  : wall_num = wall_num + self.maze.get_walls_status(self.maze.maze_cells[x-1, y])['E']
                    for i in walls_status.values(): wall_num = wall_num + i

                    if wall_num == 1:
                        self.__waypoint[(x,y)] = 0
            
            five_waypoint = []
            if len(list(self.__waypoint.keys())) >= 5: 
                five_waypoint = random.sample(list(self.__waypoint.keys()), 5)

            waypoints = {}
            for i in five_waypoint: waypoints[i] = self.__waypoint[i]
            self.__waypoint = waypoints
            self.randomwaypoint = 1
    ...
    def __colour_cell(self, cell, colour, transparency):
        ...
        x = int((cell[0] * self.CELL_W + 0.5 + 2))
        y = int((cell[1] * self.CELL_H + 0.5 + 2))
        w = int((self.CELL_W + 0.5 - 1)*0.94)
        h = int((self.CELL_H + 0.5 - 1)*0.94)
...
class Maze:
    ...
    def __init__(self, maze_cells=None, maze_size=(10,10), has_loops=True, num_portals=0, num_waypoints = 5):
    ...
    def _generate_maze(self):
        ...
        if self.has_loops:
            self.__break_random_walls(0.01)

        # if self.num_portals > 0:
        #     self.__set_random_portals(num_portal_sets=self.num_portals, set_size=2)

        # if self.num_waypoints > 0:
        #     self.__set_random_waypoints(num_waypoint_sets=self.num_waypoints, set_size=2)
    ...
    # def __set_random_waypoints(self, num_waypoint_sets, set_size=2):
    ...
    #                     dirs += dir
    #             if len(dirs) == 1:
    #                 self.__colour_cell((x, y), (125+x+y, 125+x+y, 125+x+y))
    ...
    def is_open(self, cell_id, dir):
        ...
        # print("original maze | is_open : ", self.get_walls_status(self.maze_cells[0, 0]))
