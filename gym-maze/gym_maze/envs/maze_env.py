...
class MazeEnv(gym.Env):
    ...
    def __init__(self, maze_file=None, maze_size=None, mode=None, enable_render=True):
        ...
        if maze_file:
            ...
            self.maze_view = MazeView2D(maze_name="OpenAI Gym - Maze (%d x %d)" % maze_size,\
                                        ...
                                        has_loops=has_loops, num_portals=num_portals, num_waypoints= 5, \
                                        enable_render=enable_render)
        else:
    ...
    def step(self, action):
        ...
        # print(list(self.maze_view.waypoint.values))
        for i in self.maze_view.waypoint.values():
            self.goal_waypoint = self.goal_waypoint + i

        if (self.reward_waypoint == 5) and (self.goal_waypoint == 5) and (np.array_equal(self.maze_view.robot, self.maze_view.goal)):
            reward = 1
            for i in self.maze_view.waypoint.keys():
                self.maze_view.waypoint[i] = 0
            self.reward_waypoint = 0
            done = True
        
        elif (self.goal_waypoint == 1) and (self.reward_waypoint == 0):
            reward = 0.1
            self.reward_waypoint = 1
            done = False
        
        elif (self.goal_waypoint == 2) and (self.reward_waypoint == 1):
            reward = 0.1
            self.reward_waypoint = 2
            done = False
            
        elif (self.goal_waypoint == 3) and (self.reward_waypoint == 2):
            reward = 0.1
            self.reward_waypoint = 3
            done = False
            
        elif (self.goal_waypoint == 4) and (self.reward_waypoint == 3):
            reward = 0.1
            self.reward_waypoint = 4
            done = False
        
        elif (self.goal_waypoint == 5) and (self.reward_waypoint == 4):
            reward = 0.1
            self.reward_waypoint = 5
            done = False
...
class KVRC22Random11x11(MazeEnv):
    def __init__(self, enable_render=True):
        super(KVRC22Random11x11, self).__init__(maze_size=(11, 11), mode="plus", enable_render=enable_render)
